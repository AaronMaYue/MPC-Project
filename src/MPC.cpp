 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> pside0 = CppAD::atan(3 * coeffs[3] * pow(x0, 2) + 2 * coeffs[2] * x0 + coeffs[1]);
      ...
      fg[1 + epsi_start + t] = epsi1 - ((psi0 - pside0) - (v0 * delta0/ Lf * dt));
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, double previous_delta, double previous_a){
  bool ok = true;
  //size_t i;
